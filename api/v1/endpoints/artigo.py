...
async def put_artigo(artigo_id: int, artigo: ArtigoSchema, db: AsyncSession = Depends(get_session), usurio_logado: UsuarioModel = Depends(get_current_user)):
    async with db as session:
        query = select (ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
        ...
        if artigo_up:
            ...
            if usurio_logado.id != artigo_up.usuario_id:
                artigo_up.usuario_id = usurio_logado
...
async def delete_artigo(artigo_id: int, db: AsyncSession = Depends(get_session), usurio_logado: UsuarioModel = Depends(get_current_user)):
    async with db as session:
        query = select (ArtigoModel).filter(ArtigoModel.id == artigo_id).filter(
            ArtigoModel.usuario_id == usurio_logado.id)
        result = await session.execute(query)
